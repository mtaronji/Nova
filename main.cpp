...
int main() {
    ...
    verticesResource.Preload(vertices.data(),vertices.size() * sizeof(VertexPC), vertices.size());
    indicesResource.Preload(indices.data(), sizeof(uint32_t) * indices.size(),indices.size());
    cameraResource.Preload(& ubo, sizeof(ubo), 0);

    auto mesh = Mesh(&verticesResource, &indicesResource);
